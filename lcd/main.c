...
uint32_t Dht_start(void)
{
	...
	while(1)
  {
		...
		if((TIMER_GetCounter(TIMER0) - poll_time) > 1000){return FALSE;} // cho 1ms neu khong phan hoi thi that bai 
	}
...
uint32_t DHT_Read(void)
{
	...
	if(Dht_start() == TRUE)
  {
	...
	else
	{
		...
		TIMER_Stop(TIMER0);
		return FALSE;
		LCD_String(0,0,"KHong ket noi DHT ");
	}
}
