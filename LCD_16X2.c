...
void LCD_Cmd(unsigned char _cmd){
	...
	LCD_PIN_RW=0;								
	LCD_PIN_EN=0;								//Cho chan E muc thap
	
#ifdef EIGHT_BIT_INTERFACE
	LCD_Export_8bit(_cmd);			//Xuat 4 bit thap ra D4~D7
...
void LCD_Init(void)
{
	CLK_SysTickDelay(20000);			//Delay 15ms for supply of LCD stabilize
	LCD_PIN_RS=0;
	LCD_PIN_RW=0;	
	LCD_PIN_EN = 0;
	...
	LCD_Trigger();
	DrvSYS_Delay(10);
	LCD_Cmd(EIGHT_BIT & TWO_LINE);				//address increment, cursor shift
	...
	LCD_Trigger();
	CLK_SysTickDelay(10);
	LCD_Cmd(FOUR_BIT & TWO_LINE);				//address increment, cursor shift
	
#endif	

	LCD_Cmd(DON & CURSOR_OFF & BLINK_OFF );				//Display ON, Cursor ON and Blink
	LCD_Cmd(ENTIRE_SHIFT_OFF);				//address increment, cursor shift
